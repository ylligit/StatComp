...
__StatComp20006__ is a simple R package developed to show my homeworks in the 'Statistical Computing' course. Besides, it also contains two functions _knn.est_ and _Tstat_.
...
##Function 1
Use function knn.dist() in package FNN to write a kth nearest neighbor density estimate function.
...
##Function 2
Use nonparametric method to build hypothesis test for two random vectors' independence by calculating its T value.
```{r,eval=FALSE}
...
Use leave-one-out method and bootstrap to test whether 'eruptions' and 'waiting' in dataset 'faithful' are independent.
